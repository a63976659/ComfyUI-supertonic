...
class TextToSpeech:
    ...
    def _infer(
        self, text_list: list[str], style: Style, total_step: int, speed: float = 1.0
    ) -> tuple[np.ndarray, np.ndarray]:
        assert (
    ...
    def __call__(
        ...
        segmentation_strategy: str = "standard",
    ) -> tuple[np.ndarray, np.ndarray]:
        assert (
            style.ttl.shape[0] == 1
        ), "Single speaker text to speech only supports single style"
        text_list = chunk_text(text, strategy=segmentation_strategy)
        wav_cat = None
        dur_cat = None
...
def chunk_text(text: str, max_len: int = 300, strategy: str = "standard") -> list[str]:
    """
    ...
    Args:
        ...
        strategy: Sentence splitting strategy. "standard" uses .!?/。！？; "enhanced" also treats ，、；：…… as boundaries.
    ...
    for paragraph in paragraphs:
        ...
        # Build sentence boundary pattern by strategy
        if strategy == "enhanced":
            # Include comma and other common Chinese punctuation for clearer pauses
            # English (.!?) and Chinese (。！？，、；：……) punctuation
            pattern = r"(?<!Mr\.)(?<!Mrs\.)(?<!Ms\.)(?<!Dr\.)(?<!Prof\.)(?<!Sr\.)(?<!Jr\.)(?<!Ph\.D\.)(?<!etc\.)(?<!e\.g\.)(?<!i\.e\.)(?<!vs\.)(?<!Inc\.)(?<!Ltd\.)(?<!Co\.)(?<!Corp\.)(?<!St\.)(?<!Ave\.)(?<!Blvd\.)(?<!\b[A-Z]\.)(?<=[.!?。！？，、；：……])\s*"
        else:
            # Standard sentence boundaries: English (.!?) and Chinese (。！？)
            pattern = r"(?<!Mr\.)(?<!Mrs\.)(?<!Ms\.)(?<!Dr\.)(?<!Prof\.)(?<!Sr\.)(?<!Jr\.)(?<!Ph\.D\.)(?<!etc\.)(?<!e\.g\.)(?<!i\.e\.)(?<!vs\.)(?<!Inc\.)(?<!Ltd\.)(?<!Co\.)(?<!Corp\.)(?<!St\.)(?<!Ave\.)(?<!Blvd\.)(?<!\b[A-Z]\.)(?<=[.!?。！？])\s*"

        sentences = re.split(pattern, paragraph)
        
        # If no sentence split found, treat whole paragraph as one sentence
        if len(sentences) == 1 and len(paragraph) <= max_len:
